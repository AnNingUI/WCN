...
static bool parse_strike_subtables(
    ...
    STBCBDTSubtable* subtables =
        (STBCBDTSubtable*)calloc((size_t)subtable_count, sizeof(STBCBDTSubtable));
    if (!subtables) {
        return false;
    }
...
bool stbcbdt_get_glyph_png(
    ...
    for (uint32_t i = 0u; i < font->strike.subtable_count; ++i) {
        const STBCBDTSubtable* st = &font->strike.subtables[i];
        uint32_t off0 = 0u;
        uint32_t off1 = 0u;
...
uint8_t stbcbdt_strike_bit_depth(const STBCBDTFont* font) {
    return font ? font->strike.bit_depth : 0u;
}
